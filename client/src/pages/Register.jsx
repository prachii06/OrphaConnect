...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    const isRegisteringAsAdmin = (email === adminEmail && password === adminPassword);
    ...
    try {
      ...
      if (isRegisteringAsAdmin) {
        ...
        console.log("Registering as a new Admin.");
        userData = {
          uid: user.uid,
          name,
          email,
        ...
        const makeAdmin = httpsCallable(functions, 'makeFirstAdmin');
        await makeAdmin({ email: user.email });

      } else {
        console.log("Registering as a standard user.");
        userData = {
          uid: user.uid,
          name,
          email,
      ...
      if (isRegisteringAsAdmin) {
        alert('Welcome Admin! You are now being redirected.');
        navigate('/admin-dashboard');
      } else {
        setSuccessMessage('Registration successful! Your request has been sent for approval.');
        setTimeout(() => {
          navigate('/login');
        }, 3000);
      }
      ...
      >
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <motion.button 
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 transition-all disabled:bg-indigo-400"
          >
            {loading ? 'Registering...' : 'Register'}
          </motion.button>
        </form>
